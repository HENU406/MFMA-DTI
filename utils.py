...
def load_hetero(network_path):
    ...
    drug_protein_interaction = np.loadtxt(network_path + 'mat_drug_protein.txt')  # 新增：药物-蛋白质交互矩阵
    ...
    # 新增：第5条药物元路径 - drug-protein-drug
    d_d_p = torch.tensor(np.dot(drug_protein_interaction, drug_protein_interaction.T))
    d_d_p = (d_d_p > torch.mean(d_d_p)).type(dtype=torch.int) + drug_eye
    d_d_p = sparse.csr_matrix(d_d_p).tocoo()
    ...
    d_d = dgl.heterograph({("drug", "similarity", "drug"): (drug_drug.row, drug_drug.col),
                           ...
                           ("drug", "protein", "drug"): (d_d_p.row, d_d_p.col)  # 新增第5条元路径
                           })
...
def load_zheng(network_path):
    ...
    # drug_protein_interaction = np.loadtxt(network_path + "mat_drug_target.txt")
    ...
    # # 新增：第5条药物元路径 - drug-protein-drug
    # d_d_p = torch.tensor(np.dot(drug_protein_interaction, drug_protein_interaction.T))
    # d_d_p = (d_d_p > torch.mean(d_d_p)).type(dtype=torch.int) + torch.eye(d_d_p.shape[0])
    # d_d_p = sparse.csr_matrix(d_d_p).tocoo()
    ...
    d_d = dgl.heterograph({("drug", "similarity", "drug"): (drug_drug.row, drug_drug.col),
                           ...
                           # ("drug", "protein", "drug"): (d_d_p.row, d_d_p.col)
                           })
    p_p = dgl.heterograph({("protein", "similarity", "protein"): (protein_protein.row, protein_protein.col),
...
def get_kfold_data(dataset, k=5, seed=42):
    # dataset shape: [N, 3] -> (drug_idx, prot_idx, label)
    X = dataset[:, :2].numpy()
    y = dataset[:, 2].numpy()
    ...
    for train_idx, test_idx in skf.split(X, y):
        folds.append((torch.tensor(train_idx), torch.tensor(test_idx)))
